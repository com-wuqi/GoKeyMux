...
GoKeyMux 是一个运行在 **Windows** 上的按键注入服务（键盘输入多路复用器）。它通过 gRPC 对外暴露接口，接收“按键按下 /
释放”请求，并转发到可插拔的输入注入后端。其目标是统一多种按键注入方案（驱动 / 虚拟 HID / 窗口消息）为同一个接口，方便游戏自动化、远程控制等场景。
...
| `gRPCAddress`                        | string | `localhost:50051` | gRPC 监听地址（`tcp4`）                                |
